function FTHA(Init::Dict)
    ...
    for i in 1:length(Init["SIMUL"]["α"])-1
        if abs(Init["SIMUL"]["𝕧"][i] - Init["SIMUL"]["𝕧"][i+1]) <= Init["TOL"]["ϵ_v"]
            ...
            push!(Init["SIMUL"]["T"], temp_u(Init["SIMUL"]["u"][end], Init["SIMUL"]["u"][end-1], Init["SIMUL"]["T"][end]))
            push!(Init["SIMUL"]["P"], press_u(Init["SIMUL"]["T"][end], Init["SIMUL"]["𝕧"][i]))
        else
            j = 0
            ...
            while j == 0 || abs(w_ii[end] - w_ii[end-1]) >= sqrt(Init["TOL"]["ϵ_w"])
                ...
                push!(T_ii, temp_u(u_ii[end], Init["SIMUL"]["u"][i], Init["SIMUL"]["T"][i]))
                push!(P_ii, press_u(T_ii[end], Init["SIMUL"]["𝕧"][i]))
